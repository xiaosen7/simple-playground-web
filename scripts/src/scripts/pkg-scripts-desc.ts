...
async function genForProject(project: IProject) {
  ...
  if (startAnchorIndex < 0 || endAnchorIndex < 0) {
    console.warn(
      `${getRelativeFromPkgRoot(project.readme.filePath)} 起始位置或者结束位置未找到`
    );
    return;
  }
...
async function genMdString(project: IProject) {
  const builder = markdown
    .newTableBuilder(0, 3)
    .header(["Name", "Description", "Content", "Run"]);
  ...
  Object.entries(project.packageScriptsJSON.content)
    ...
    .forEach(([name, desc]) =>
      builder.appendRow([
        ...
        project.dir === PKG_ROOT
          ? `\`pnpm -w run ${name}\``
          : `\`pnpm --filter="${project.manifest.name}" run ${name}\``,
      ])
    );
